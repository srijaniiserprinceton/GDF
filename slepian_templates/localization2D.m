...
elseif strcmp(XY,'VDF_cartesian')
  N=double(N);
  defval('method','GL')
  fname_XY='/Users/srijanbharatidas/Documents/Research/SpaceWeather/VDF_projects/VDF_Paper1_ESA/XY_pts.mat';
  load(fname_XY);
  XY=[X(:) Y(:)];
  fname_XYP='/Users/srijanbharatidas/Documents/Research/SpaceWeather/VDF_projects/VDF_Paper1_ESA/XYP.mat';
  load(fname_XYP);
  XYP=[XP(:) YP(:)];
  [G,H,V,K,XYP]=localization2D(XY,N,[],method,[],[],[10 10],[]);
  % demoplots1(G,H,V,K,XYP,XY)

  % saving the Cartesian Slepians 
  % fname=fullfile(getenv('IFILES'), 'LOCALIZATION2D',...
	% 	         sprintf('localization2D-%i.mat',...
	% 		     N));
  % 
  % save(fname,'-v7.3','G','H','V','K','XYP','XY');
  varns={G,H,V,K,XYP,XY};
  varargout=varns(1:nargout);
