...
def main(start_idx = 0, Nsteps = None, MCMC = False, MCMC_WALKERS=8, MCMC_STEPS=2000, MIN_METHOD='L-BFGS-B', SAVE_FIGS=False):
    ...
    write_pickle(vdf_rec_bundle, f'./paper_plots/mcmc_walker_step/scipy_vdf_rec_data_{MCMC_WALKERS}_{MCMC_STEPS}_{ymd}_{a_label}_{b_label}')

def run(trange, psp_vdf, creds, config):
    global gvdf_tstamp
...
if __name__=='__main__':
    ...
    # Generate the psp_vdf once and only once!
    psp_vdf = fn.init_psp_vdf(trange, CREDENTIALS=creds, CLIP=True)
    psp_old = psp_vdf.copy()

    config_preamble = 'init_params'
    indecies = np.arange(1,22)
    config_names = [f'{config_preamble}{index}' for index in indecies]

    for config_name in config_names:
        config = importlib.import_module(f'initializations.{config_name}')
        print(f"[INFO] Using config: {config_name}")
        run(trange, psp_vdf, creds, config)
        psp_vdf = psp_old
