...
def convert_logresf_to_f(log_scaled_f_norm, gvdf_tstamp, vpara_grid, vperp_grid):
    if(not isinstance(log_scaled_f_norm, tuple)):
        if(len(log_scaled_f_norm) != len(vpara_grid)):
            vpara = np.concatenate([vpara_grid, vpara_grid])
            vperp = np.concatenate([-vperp_grid, vperp_grid])
        else:
            vpara = vpara_grid * 1.0
        ...
        f = []
        for i in range(len(log_scaled_f_norm)):
            log_scaled_f_norm_i = log_scaled_f_norm[i]
            if(len(log_scaled_f_norm_i) != len(vpara_grid)):
                vpara = np.concatenate([vpara_grid, vpara_grid])
                vperp = np.concatenate([-vperp_grid, vperp_grid])
            else:
                # reshaping grids for plotting
                if(vperp_grid.shape[0] == (gvdf_tstamp.nptsx * gvdf_tstamp.nptsy)):
                    xx = np.reshape(vperp_grid, (gvdf_tstamp.nptsx, gvdf_tstamp.nptsy), 'F')
                    yy = np.reshape(vpara_grid, (gvdf_tstamp.nptsx, gvdf_tstamp.nptsy), 'F')

                    vperp = xx.flatten() * 1.0
                    vpara = yy.flatten() * 1.0
                else:
                    vpara = vpara_grid * 1.0
                    vperp = vperp_grid * 1.0
            ...
            scaled_f = np.power(10, log_scaled_f)
            f.append(scaled_f * (M - gvdf_tstamp.epsilon + 1) - 1)
