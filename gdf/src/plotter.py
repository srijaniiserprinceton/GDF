...
def plot_Lcurve_knee_HYBRID(tidx, model_misfit, data_misfit, knee_idx, mu_arr, ext='png', SAVE=False):
    if knee_idx:
        mu = mu_arr[knee_idx]
        fig = plt.figure()
        plt.plot(model_misfit, data_misfit, 'b')
        ...
        print("Lambda assigned in config.")
...
def hybrid_plotter(gvdf_tstamp, vdf_inv, vdf_super, tidx,
    ...
    plot_Lcurve_knee_HYBRID(tidx, model_misfit, data_misfit, gvdf_tstamp.lambda_knee_idx,
                            gvdf_tstamp.lambda_arr, SAVE=SAVE_FIGS)
