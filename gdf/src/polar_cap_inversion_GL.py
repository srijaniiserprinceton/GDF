...
def get_Slepians(gvdf_tstamp, tidx):
    gvdf_tstamp.S_alpha_n = basis_fn.get_Slepians_scipy(gvdf_tstamp.Slep.C, gvdf_tstamp.theta_nonan, 
...
def get_G_matrix(gvdf_tstamp):
    ...
    npoints = len(gvdf_tstamp.vpara_nonan)
    gvdf_tstamp.G_k_n = np.reshape(gvdf_tstamp.G_i_alpha_n, (-1, npoints))

def inversion(gvdf_tstamp, vdfdata, tidx):
    """
    ...
    get_Bsplines(gvdf_tstamp)
    get_Slepians(gvdf_tstamp, tidx)
    get_G_matrix(gvdf_tstamp)
...
def super_Bsplines(gvdf_tstamp):
    gvdf_tstamp.super_B_i_n = basis_fn.get_Bsplines_scipy(gvdf_tstamp.knots, gvdf_tstamp.p, gvdf_tstamp.super_rfac)
