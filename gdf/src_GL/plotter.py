...
def plot_span_vs_rec_contour_POLCAP(gvdf, vdf_data, vdf_rec, tidx, GRID=False, VA=None, SAVE=False, ext='png'):
    if VA:
        v_para_all = np.concatenate([gvdf.vpara_nonan_inst, gvdf.vpara_nonan_inst])/VA
        v_perp_all = np.concatenate([-gvdf.vperp_nonan_inst, gvdf.vperp_nonan_inst])/VA
        xlabel = r'$v_{\perp}/v_{A}$'
        ylabel = r'$v_{\parallel}/v_{A}$'
    else:
        v_para_all = np.concatenate([gvdf.vpara_nonan_inst, gvdf.vpara_nonan_inst])
        v_perp_all = np.concatenate([-gvdf.vperp_nonan_inst, gvdf.vperp_nonan_inst])
...
def plot_super_resolution_POLCAP(gvdf, vdf_super, mu, tidx, SAVE=False, VDFUNITS=False, VSHIFT=None, DENSITY=None, ext='png'):
    ...
    span_gridx = np.append(-gvdf.vperp_nonan_inst, gvdf.vperp_nonan_inst)
    span_gridy = np.append(gvdf.vpara_nonan_inst, gvdf.vpara_nonan_inst)
    ...
    if VDFUNITS:
        ...
        if VSHIFT:
            ...
            ax.scatter(gvdf.vperp_nonan_inst, gvdf.vpara_nonan_inst - VSHIFT, c=np.log10(f_data), s=30, cmap='inferno', norm=norm, edgecolor='k', linewidths=0.5)
            ...
            ax.scatter(gvdf.vperp_nonan_inst, gvdf.vpara_nonan_inst, c=np.log10(f_data), s=30, cmap='inferno', norm=norm, edgecolor='k', linewidths=0.5)

    else:
        ax1 = ax.tricontourf(grids[mask,1], grids[mask,0], vdf_super[mask], levels=np.linspace(0,4.0,10), cmap='inferno')
        ax.scatter(gvdf.vperp_nonan_inst, gvdf.vpara_nonan_inst, c=(gvdf.vdfdata), s=30, cmap='inferno', norm=norm, edgecolor='k', linewidths=0.5)
...
def plot_Lcurve_knee_HYBRID(tidx, model_misfit, data_misfit, knee_idx, mu, ext='png', SAVE=False):
    if knee_idx:
        ...
        print("Lambda was assigned in config. ")
