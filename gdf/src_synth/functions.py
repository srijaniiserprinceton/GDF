...
from gdf.src_synth.utils import _cdf_to_xarray, _convert_to_astropy
...
def init_psp_moms(trange, CREDENTIALS=None, CLIP=False):
    ...
    xr_time_object = _convert_to_astropy(xr_data.Epoch.data)
    xr_time_array = np.datetime64(xr_time_object.utc.datetime, dtype='datetime64[ns]')
...
def vdf_moments(gvdf, vdf_super, tidx):
    ...
    # Calculate the heat flux integrals
    # We note that in a gyrotropic FAC:
    # Q_ijk = A b_i b_j b_k + B (b_i D_jk + b_j D_ik + b_k D_ij), where b_{i,j,k} are unit vectors and
    # A = Q_iii = m int (v_para - u_para)^3 f(v_para, v_perp) d^3v
    # B = Q_ijj = m int (v_para - u_para) v_perp^2 f(v_para, v_perp) d^3v
    # All other combinations go to zero.
    # Finally, there are 27 components for ijk, however, most of them reduce to 0!
    A = m_p*(2*np.pi*np.sum((grids[mask,1][mask2] * 1e5 - vpara)**3 * grids[mask,0][mask2]*1e5 * f_super[mask][mask2] * dx*1e5 * dy*1e5)/density)
    B = m_p*(2*np.pi*np.sum((grids[mask,1][mask2] * 1e5 - vpara) * (grids[mask,0][mask2]*1e5)**3 * f_super[mask][mask2] * dx*1e5 * dy*1e5)/density)

    heat_flux_vector = 0.5 * (A + 2*B)
    heat_flux_comps = [A, B]

    return(density, vpara/1e5, T_comp, T_trace, heat_flux_comps, heat_flux_vector)

def vdf_moments_new(gvdf, vdf_super, tidx):
    """
    Compute plasma moments (density, bulk velocity, and temperatures) 
    from a gyrotropic velocity distribution function (VDF).

    Parameters
    ----------
    gvdf : object
        Object containing VDF metadata and grid definitions. Must have:
            gvdf.grid_points  -> array of shape (Npts, 2), [v_perp, v_para]
            gvdf.hull_mask    -> boolean mask for valid VDF region
            gvdf.minval[tidx] -> scalar minimum VDF scaling value
            gvdf.maxval[tidx] -> scalar maximum VDF scaling value
            gvdf.nptsx, gvdf.nptsy -> grid dimensions
    vdf_super : np.ndarray or tuple
        Log-scaled (base-10) VDF values or tuple (polar_vdf, cartesian_vdf).
        The function will use the Cartesian one if a tuple is provided.
    tidx : int
        Time index into gvdf.minval / gvdf.maxval arrays.
    ...
    vpara : float
        Field-aligned bulk velocity in km/s.
    (T_para, T_perp) : tuple of floats
        Parallel and perpendicular temperatures in K.
    T_trace : float
        Scalar temperature (trace) in K.
    """

    # --- Handle tuple input (hybrid case)
    if isinstance(vdf_super, tuple):
        # select the Cartesian super-resolution version
        vdf_super = vdf_super[1]
        vdfT = np.reshape(vdf_super, (gvdf.nptsx, gvdf.nptsy))
        vdf_super = np.transpose(vdfT).flatten()

    # --- Extract VDF scaling values and grid info
    minval = gvdf.minval[tidx]
    maxval = gvdf.maxval[tidx]
    grids  = gvdf.grid_points

    v_perp = np.reshape(grids[:, 0], (gvdf.nptsx, gvdf.nptsy))
    v_para = np.reshape(grids[:, 1], (gvdf.nptsx, gvdf.nptsy))

    dv_perp = v_perp[0, 1] - v_perp[0, 0]
    dv_para = v_para[1, 0] - v_para[0, 0]

    # --- Valid data mask (inside convex hull and positive v_perp)
    mask = gvdf.hull_mask
    valid = mask & (grids[:, 0] >= 0)

    # --- Convert from log10(VDF) to linear space and apply limits
    f = np.power(10, vdf_super) * minval
    f[f > 5 * maxval] = 0.0  # remove unphysical outliers
    ...
    # --- Define shorthand arrays for integration
    vperp = grids[valid, 0] * km_to_cm
    vpara = grids[valid, 1] * km_to_cm
    fval  = f[valid]
    dV    = dv_perp * dv_para * (km_to_cm ** 2)

    # --- Density: n = 2π ∫ v_perp f dv_perp dv_para
    density = 2 * np.pi * np.sum(vperp * fval * dV)

    # --- Bulk parallel velocity: u_|| = (2π / n) ∫ v_|| v_perp f dv_perp dv_para
    upara = (2 * np.pi / density) * np.sum(vpara * vperp * fval * dV)

    # --- Parallel temperature: T_|| = (m_p / k_B n) 2π ∫ (v_|| - u_||)^2 v_perp f dv_perp dv_para
    T_para = (m_p / (k_b * density)) * 2 * np.pi * np.sum((vpara - upara)**2 * vperp * fval * dV)

    # --- Perpendicular temperature: T_⊥ = (m_p / 2k_B n) 2π ∫ v_⊥^3 f dv_perp dv_para
    T_perp = (m_p / (2 * k_b * density)) * 2 * np.pi * np.sum(vperp**3 * fval * dV)

    # --- Trace (isotropic-equivalent) temperature
    T_trace = (T_para + 2 * T_perp) / 3
    ...
    # A = parallel cubic: ∫ (v_para - u_para)^3 v_perp f dv_perp dv_para
    A = m_p * (2 * np.pi / density) * np.sum((vpara - upara)**3 * vperp * fval * dV)

    # B = perpendicular contribution: ∫ (v_para - u_para) v_perp^2 v_perp f dv_perp dv_para
    B = m_p * (2 * np.pi / density) * np.sum((vpara - upara) * vperp**3 * fval * dV)

    # Combined field-aligned heat flux
    heat_flux_vector = 0.5 * (A + 2 * B)
    heat_flux_comps = [A, B]

    # --- Return moments
    return density, upara / km_to_cm, (T_para, T_perp), T_trace, heat_flux_comps, heat_flux_vector
