...
class gyrovdf:
    ...
    def init_polcap_params(self, polcap_params):
        ...
        if(polcap_params['TH'] is None):
            ...
            self.get_coors_supres = self.get_coors_single_TH
    ...
    def get_coors_single_TH(self, u_bulk, tidx):
        r"""
    ...
    def get_coors_update_TH(self, u_bulk, tidx):
        ...
        # Boosting along vpara [this step is crucial for the polar cap method, only]
        self.vshift = self.velocity[tidx, *self.max_indices[tidx]] + self.vpara[*self.max_indices[tidx]]
        self.vpara -= self.vshift
...
def main(START_INDEX = 0, NSTEPS = None, NPTS_SUPER=49,
    ...
    for tidx in tqdm(range(START_INDEX, START_INDEX + NSTEPS)):
        ...
        if(SAVE_SUPRES):
            ...
            supres_bundle['vpara_supres'] = gvdf_tstamp.grid_points[:,1] - gvdf_tstamp.vshift
            ...
            supres_bundle['vpara_data'] = gvdf_tstamp.vpara_nonan[~gvdf_tstamp.fid_mask] - gvdf_tstamp.vshift
