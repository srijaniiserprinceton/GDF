...
class gyrovdf:
    ...
    def setup_timewindow(self, vdf_dict, trange, CREDENTIALS=None, CLIP=False):
        ...
        self.l3_time = time #data.Epoch.data     # check to make sure the span moments match the l2 data!
        self.l2_time = time
    ...
    def inversion(self, u_bulk, vdfdata, tidx, SUPER=False, NPTS=101):
            ...
            def super_inversion(vdfdata):
                ...
                for mu in mu_arr:
                    coeffs = np.linalg.inv(G_g + mu * self.D) @ self.G_k_n @ vdfdata
                    ...
                    model_misfit.append(np.linalg.norm(coeffs @ self.D @ coeffs))
                ...
                # coeffs = np.linalg.inv(G_g + self.mu * I) @ self.G_k_n @ vdfdata
                coeffs = np.linalg.inv(G_g + mu_arr[knee_idx] * self.D) @ self.G_k_n @ vdfdata
...
def main(start_idx = 0, Nsteps = None, NPTS_SUPER=101, MCMC = False, MCMC_WALKERS=8, MCMC_STEPS=2000, MIN_METHOD='L-BFGS-B', SAVE_FIGS=False, SAVE_PKL=True):
    ...
    for tidx in tqdm(range(start_idx, start_idx + Nsteps)):
        ...
        vdfdata = np.log10((psp_vdf.vdf.data[tidx, gvdf_tstamp.nanmask[tidx]]*1e-30)/gvdf_tstamp.minval[tidx])
        ...
        print("HERE", threeD_points.shape, vdfdata.shape, gvdf_tstamp.bvec[tidx], u_origin)
    ...
    if(SAVE_PKL):
        misc_fn.write_pickle(vdf_rec_bundle, f'./Outputs/biMax_vdf_rec_data_{MCMC_WALKERS}_{MCMC_STEPS}_{ymd}_{a_label}_{b_label}')
